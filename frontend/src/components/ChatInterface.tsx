...
// Update the mockProductSuggestions with better placeholder images
const mockProductSuggestions: ProductSuggestion[] = [
  {
    id: "1",
    title: "Lightweight Packable Jacket",
    price: "£59",
    imageUrl: "/placeholder.svg?height=300&width=300&text=Lightweight+Jacket",
  },
  {
    id: "2",
    title: "Travel Windbreaker",
    price: "£35",
    imageUrl: "/placeholder.svg?height=300&width=300&text=Travel+Jacket",
  },
  {
    id: "3",
    title: "Compact Rain Jacket",
    price: "£15",
    imageUrl: "/placeholder.svg?height=300&width=300&text=Rain+Jacket",
  },
  {
    id: "4",
    title: "Foldable Windbreaker",
    price: "£28",
    imageUrl: "/placeholder.svg?height=300&width=300&text=Windbreaker",
  },
  {
    id: "5",
    title: "Ultralight Shell",
    price: "£42",
    imageUrl: "/placeholder.svg?height=300&width=300&text=Shell+Jacket",
  },
]
...
const ChatInterface = ({ type, onComplete, onViewDraftListing }: ChatInterfaceProps) => {
    ...
    setTimeout(() => {
      ...
      if (type === "sell") {
        if (chatMessages.length === 1) {
          ...
          aiResponse =
            "Perfect! What price are you thinking? I can help suggest a competitive price based on similar items."
        } else if (chatMessages.length === 5) {
          aiResponse =
            "Excellent! I've gathered all the information needed. Your listing looks great and should attract buyers quickly!"
        ...
        aiResponse = "I've searched current listings and here are some matches, tap to view"
      ...
      // Show product suggestions for buy chat
  ...
  const handleProductClick = (product: ProductSuggestion) => {
    ...
    // Handle product click - could navigate to product page or show details
    if (onViewDraftListing) {
      onViewDraftListing()
    }
  }
  ...
  const quickReplies =
    type === "sell"
      ? ["It's in excellent condition", "It's gently used", "It has some wear", "It's brand new"]
      : ["Show me electronics", "Looking for furniture", "Need clothing items", "Show me everything"]
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {chatMessages.map((message) => (
          ...
          //   <div
          //     className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${message.sender === "user" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-900"
          //       }`}
          //   >
          ...
          //   </div>
          // </div>
          <ChatMessage key={message.id} message={message} onProductClick={handleProductClick} />
        ))}

        {/* Product Suggestions Carousel */}
        {type === "buy" && showProductSuggestions && (
          <div className="flex justify-start">
            <div className="bg-gray-100 text-gray-900 px-4 py-3 rounded-lg max-w-full">
              <ProductSuggestionCarousel suggestions={mockProductSuggestions} onProductClick={handleProductClick} />
            </div>
          </div>
        )}
